...
When editing pack behavior, edit the payload directories directly. The pack's invariants (no in-process subagents in the payload, roles and gate names instead of tool names, structural review independence that no configuration relaxes), its Apache-2.0 licensing and `NOTICE` duties, and the changelog and validation protocol are listed in `CLAUDE.md` — they apply to any agent working on this repo.
