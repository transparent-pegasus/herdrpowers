...
- **Roles and gates, not tool names.** Skills speak in roles (Orchestrator, Reviewer, Coder, Generalist) and gate names (`task-review`, `final-branch-review`, …). Assignments and gates resolve at runtime from the target repo's `.herdrpowers/config.yaml` over the shipped defaults in `skills/orchestration/roles.yaml`. `roles.yaml` is the **default layer only** — never the customization point, because plugin installs are read-only. Adding a role or a gate means updating `roles.yaml`, the gate table in `skills/orchestration/SKILL.md`, the template and tables in `commands/init.md`, and `docs/roles.md`.
- **Independence is structural, and not configurable.** The pane that writes code never reviews it; the reviewing pane audits the test code, because the implementing pane wrote it. Do not weaken either half without replacing the other, and never let a config value relax it — a disabled gate removes a review, it never converts one into a self-review. A disabled gate is named in the workflow's final report.
